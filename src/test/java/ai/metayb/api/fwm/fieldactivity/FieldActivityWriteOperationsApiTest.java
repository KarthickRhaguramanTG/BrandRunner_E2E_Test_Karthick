...
public class FieldActivityWriteOperationsApiTest extends BaseApiTest {
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity (List Monthwise Activities, 2 variants) returns the CloudFront/S3 SPA fallback, not JSON")
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity/campaigns-table (2 variants) returns the CloudFront/S3 SPA fallback, not JSON")
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity/:id/assign (Assign Field Users, 7 variants) returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Assign Field Users")
    @Description("Blocked by environment - directly verified live. See class Javadoc. Not executed.")
    public void assignFieldUsersNotAutomatedDueToEnvironmentIssue() {
    }

    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: PATCH /web/field-activity/:id/assign (Update Field Assignment, 4 variants) returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Update Field Assignment")
    @Description("Blocked by environment - same route family. See class Javadoc. Not executed.")
    public void updateFieldAssignmentNotAutomatedDueToEnvironmentIssue() {
    }

    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: DELETE /web/field-activity/delete/:budgetId/:date returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Delete by Budget ID / Date")
    @Description("Blocked by environment - directly verified live. See class Javadoc. Not executed.")
    public void deleteByBudgetIdDateNotAutomatedDueToEnvironmentIssue() {
    }

    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity/media/upload returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Upload Media")
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity/attendance-regularization returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Attendance Regularization")
    @Description("Blocked by environment - same systemic root cause. Not executed.")
    public void attendanceRegularizationNotAutomatedDueToEnvironmentIssue() {
    }

    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST .../supervisor-reconcile/send-otp, verify-otp, submit return the CloudFront/S3 SPA fallback, not JSON")
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST .../promoter-close-sale/send-otp, verify-otp, submit return the CloudFront/S3 SPA fallback, not JSON")
    ...
    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: PATCH /web/field-activity/:id returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Update Field Activity")
    @Description("Blocked by environment - directly verified live. See class Javadoc. Not executed.")
    public void updateFieldActivityNotAutomatedDueToEnvironmentIssue() {
    }

    @Test(enabled = false, groups = {"api", "regression"}, description = "BLOCKED: POST /web/field-activity/field-users returns the CloudFront/S3 SPA fallback, not JSON")
    @Story("Field Users")
    @Description("Blocked by environment - directly verified live. See class Javadoc. Not executed.")
